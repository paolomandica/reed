...
- 2026-09-15 — v0.4.0 prepared: Misaki G2P, British voices, prompt guide, HTML-input/Markdown-output removal, CI build fixes (Cython 3.2.x build constraint). Checks green (ruff, mypy, 66 tests); CI green on Ubuntu + macOS.
- 2026-09-15 — v0.4.0 released: tag `v0.4.0` pushed, GitHub release created with the wheel + sdist attached, and `reed-cli` 0.4.0 published to PyPI by the user. Fresh `uv tool install --force reed-cli` verified end to end (`reed --version` → 0.4.0, `reed doctor` all green including `misaki[en]`, `reed demo --no-audiobook` → EPUB).
